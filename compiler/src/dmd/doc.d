...
final class ParamSection : Section
{
    override void write(Loc loc, DocComment* dc, Scope* sc, Dsymbols* a, ref OutBuffer buf)
    {
                ...
                buf.writestring("$(DDOC_PARAM_ROW ");
                {
                    buf.writestring("$(DDOC_PARAM_ID ");
                    {
                        ...
                        else
                        {
                            ...
                            {
                                sc.eSink.message(s.loc, "Ddoc: function declaration has no parameter '%.*s'", cast(int)namelen, namestart);
                            }
        ...
        if (tf)
        {
            ...
            {
                sc.eSink.message(s.loc, "Ddoc: parameter count mismatch, expected %llu, got %llu",
                        cast(ulong) pcount, cast(ulong) paramcount);
                if (paramcount == 0)
                {
                    // Chances are someone messed up the format
                    sc.eSink.message(s.loc, "       Note that the format is `param = description`");
                }
            }
        }
    }
    ...
    for (size_t u = start; u < buf.length; u++)
    {
        ...
        case ')':
            if (!inCode)
            {
                ...
                {
                    //stray ')'
                    eSink.message(loc, "Ddoc: Stray ')'. This may cause incorrect Ddoc output. Use $(RPAREN) instead for unpaired right parentheses.");
                    buf.remove(u, 1); //remove the )
                    buf.insert(u, "$(RPAREN)"); //insert this instead
                    u += 8; //skip over newly inserted macro
                }
    ...
    if (par_open) // if any unmatched lparens
    {
        ...
        for (size_t u = buf.length; u > start;)
        {
            ...
            case '(':
                ...
                {
                    //stray '('
                    eSink.message(loc, "Ddoc: Stray '('. This may cause incorrect Ddoc output. Use $(LPAREN) instead for unpaired left parentheses.");
                    buf.remove(u, 1); //remove the (
                    buf.insert(u, "$(LPAREN)"); //insert this instead
                }
                else
